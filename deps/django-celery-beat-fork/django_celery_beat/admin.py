...
class PeriodicTaskAdmin(admin.ModelAdmin):
    ...
    @admin.action(
        description=_('Stop selected tasks')
    )
    def stop_tasks(self, request, queryset):
        names = set([o['name'] for o in queryset.values('name')])
        active_ids = []

        for k, v in current_app.control.inspect().active().items():
            for t in v:
                if t['name'] in names:
                    active_ids.append(t['id'])
        current_app.control.terminate(active_ids)
        self.message_user(
            request,
            '{0} active_ids found and terminated'.format(len(active_ids)),
        )
